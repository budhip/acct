...
type QueueUnicornClient interface {
...
func New(cfg *config.Configuration) (QueueUnicornClient, error) {
	queueUnicornClient, err := queueunicorn.NewGoQueueServiceClient(queueunicorn.Hostname(cfg.GoQueueUnicorn.BaseURL), queueunicorn.UseHttps(true))
	if err != nil {
